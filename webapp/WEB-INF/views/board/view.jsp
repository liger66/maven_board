...
<script>
		...
		if(confirm('정말로 삭제하시겠습니까?')){
			$.ajax({
				...
				success:function(data){
					//alert("삭제가 완료 되었습니다. 메인페이지로 이동합니다.");
	...
	function send(f){
		if(/^.{2,10}$/.test(f.name.value) == false){
			alert("이름은 2 ~ 10글자로 입력해 주세요.");
			f.name.focus();
			return;
		}
		
		if(/^.{1,100}$/.test(f.content.value) == false){
			alert("내용은 1 ~ 100글자로 입력해 주세요.");
			f.content.focus();
			return;
		}
	...
	<div class="container">
		...
		<div class="body">
			...
			<div class="buttons">
				...
				<button type="button" onclick="del(${board.id})">삭제</button>
				<button type="button" onclick="
					location.href='${pageContext.request.contextPath}/board/comment?id=${board.id}';
				">답글</button>
				<button type="button" onclick="modify(${board.id})">수정</button>
			</div>
		</div>
		<div class="footer">
			<div class="reply">
